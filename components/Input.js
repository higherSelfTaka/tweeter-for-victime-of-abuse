...
return (
<div className={`border-b border-gray-800 p-3 flex space-x-3 overflow-y-scroll 
    ${loading && "opacity-60"}`}>
    ...
    <img src={session.user.image} alt="" className="h-11 w-11 rounded-full cursor-pointer"/>
     <div className="w-full divide-y divide-gray-800">
       ...
       <div classNanme={`${selectedFile && "pb-7"} ${input && "space-y-2.5"}`}>
        <textarea 
        value={input}  
  ...
  {!loading && (
        ...
        <div className="flex items-center">
          ...
          <RadioIcon className="text-[#1d9bf0] h-[22px]"/>
          </div>
